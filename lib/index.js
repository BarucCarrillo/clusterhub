...
export const getNotifications = async (id) => {
  try {
    const response = await fetch(`${BACKEND_API}/notifications/${id}`, {
      method: "GET",
      headers: {
        "Content-Type": "application/json",
      },
...
export const deleteNotificationById = async (id) => {
  try {
    ...
    if (!response.ok) {
      const errorData = await response.json();
      throw new Error(errorData.error || "Notification deletion failed");
    }
...
export const getRecomendations = async (id) => {
  try {
    const response = await fetch(`${BACKEND_API}/recomendations/${id}`, {
      method: "GET",
      headers: {
        "Content-Type": "application/json",
      },
